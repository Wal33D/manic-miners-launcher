 ...
 */
export function useIpcListener(channel: string, handler: (data: any) => void, deps: React.DependencyList = []) {
 ...
 * @param deps - Dependencies array for the effect
 */
export function useIpcOnce(channel: string, handler: (data: any) => void, deps: React.DependencyList = []) {
  useEffect(() => {
    if (!window.electronAPI) return;
